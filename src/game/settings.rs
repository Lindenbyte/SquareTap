...
impl Settings {
    ...
    pub fn render(&mut self, font: Font) {
        ...
        draw_text_ex(
            "Key bindnings:", 
            250.0, screen_height()/2.0 - 175.0, 
            TextParams{ font, font_size: 32, color: WHITE, ..Default::default() }
        );
        
        draw_text_ex(
            "Select: Enter", 
            250.0, screen_height()/2.0 - 125.0, 
            TextParams{ font, font_size: 18, color: WHITE, ..Default::default() }
        );
        draw_text_ex(
            "Back: Escape",
